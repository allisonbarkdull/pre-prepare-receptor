...
def get_ligand_smiles(ligand_id: str) -> Optional[str]:
    ...
    """
    api_url = f"https://data.rcsb.org/rest/v1/core/chemcomp/{ligand_id.upper()}"
    try:
        r = requests.get(api_url, timeout=10)
    except Exception as e:
        logging.warning(f"Failed to reach RCSB for {ligand_id}: {e}")
        return None
    if r.status_code != 200:
        logging.warning(f"Ligand {ligand_id} not found on RCSB (status {r.status_code}).")
        return None
    data = r.json()
    # RCSB stores descriptor in multiple places; try common path
    smiles = None
    for key in ("rcsb_chem_comp_descriptor", "purchasable_compound", "chem_comp"):
        if key in data and isinstance(data[key], dict):
            smiles = data[key].get("smiles") or data[key].get("description")
            if smiles:
                break
...
def make_sdf_from_residue(pdb_file: str, resname: str, chain: str, output_sdf: str, smiles: Optional[str] = None) -> str:
    """Extract a residue and save as SDF."""
    ...
    try:
        ...
        # Skip scrubber if SMILES already has explicit H
        mol_has_H = "H" in smiles  # simple heuristic
        if mol_has_H:
            logging.info(f"SMILES for {resname} has explicit hydrogens; skipping scrubber.")
...
# ---------------------------
def ligand_coords_from_sdf(sdf_path: str) -> np.ndarray:
    suppl = Chem.SDMolSupplier(sdf_path, removeHs=False)
    mol = next((m for m in suppl if m is not None), None)
    if mol is None or mol.GetNumConformers() == 0:
        raise ValueError(f"No conformer found in {sdf_path}")
...
def select_neighborhood(receptor: pr.AtomGroup, args) -> Tuple[Dict[str,List[ResidueKey]], str]:
    ...
    if args.box_center and args.box_lengths:
        ...
        lig_coords = ligand_coords_from_sdf(args.ligand_sdf)
...
def main():
    ...
    parser.add_argument("--ligand_sdf", type=str, default=None,
                        help="Ligand SDF path. Input either a ligand sdf or ligand_resname + ligand_chain")
    ...
    if args.mode == "step0":
        ...
        if args.ligand_sdf:
            ...
            print("[ERROR] Step 0 requires one of: --ligand_sdf, (--ligand_resname + --ligand_chain), or "
        ...
        ligands_to_parametrize: List[tuple[str, str]] = []
        cofactor_resnames = []
        ...
        if args.cofactor:
            for entry in args.cofactor:
                ...
                if os.path.isfile(val) and val.lower().endswith((".sdf", ".mol2", ".smi")):
